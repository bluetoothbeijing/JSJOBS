...
<div id="jsjobs-wrapper">
    ...
    <div id="jsjobs-content">
        ...
        <div id="jsjobs-head">
            ...
            <a onclick="Joomla.submitbutton('salaryrange.editjobsalaryrange');" class="jsjobs-add-link green-bg button" title="<?php echo Text::_('Add New').' '.Text::_('Salary Range'); ?>">
                <img alt="<?php echo Text::_('Plus Icon'); ?>" src="components/com_jsjobs/include/images/plus.png"/>
                <?php echo Text::_('Add New').' '.Text::_('Salary Range'); ?>
            </a>
        </div>
        <div id="jsstadmin-data-wrp" class="p0 bg-n bs-n">
      <form action="index.php" method="post" name="adminForm" id="adminForm">
        <div id="jsjobs_filter_wrapper">           
            <span class="jsjobs-filter"><input type="text" name="searchrangestart" placeholder="<?php echo Text::_('Salary Range Start'); ?>" id="searchrangestart" value="<?php if (isset($this->lists['searchrangestart'])) echo $this->lists['searchrangestart']; ?>"  /></span>
            <span class="jsjobs-filter"><input type="text" name="searchrangeend" placeholder="<?php echo Text::_('Salary Range End'); ?>" id="searchrangeend" value="<?php if (isset($this->lists['searchrangeend'])) echo $this->lists['searchrangeend']; ?>"  /></span>
            ...
            <span class="jsjobs-filter jsjobs-filter-btn-wrp"><button class="js-button js-form-reset" onclick="document.getElementById('searchrangestart').value = '';document.getElementById('searchrangeend').value = '';document.getElementById('searchstatus').value = ''; this.form.submit();"><?php echo Text::_('Reset'); ?></button></span>
        </div>
                ...
                <table class="adminlist" border="0" id="js-table">
                    <thead>
                        <tr>
                            ...
                            <th width="60%" class="left"><?php echo Text::_('Salary Range'); ?></th>
                    ...
                    for ($i = 0, $n = count($this->items); $i < $n; $i++) {
                        ...
                        $link = OutputFilter::ampReplace('index.php?option=' . $this->option . '&task=salaryrange.editjobsalaryrange&cid[]=' . $row->id);
                        ?>
                        <tr valign="top" class="<?php echo "row$k"; ?>">
                            ...
                            <td align="center">
                                <?php
                                $img = ($row->status) ? 'tick.png' : 'cross.png';
                                $status_value = ($row->status) ? 0 : 1;
                                $status_link = OutputFilter::ampReplace('index.php?option='.$this->option.'&task=common.makePublishUnpublish&for=salaryrange&status='.$status_value.'&cid[]='.$row->id);
                                ?>
