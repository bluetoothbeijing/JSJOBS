...
//layout start
        if ($layoutName == 'formcareerlevels') {          // jobtypes
            ...
            ToolbarHelper::title(Text::_('Job Career Levels') . ': <small><small>[ ' . $text . ' ]</small></small>');
            ToolbarHelper::apply('careerlevel.savejobcareerlevelsave', 'Save');
            ToolbarHelper::save2new('careerlevel.savejobcareerlevelandnew');
            ToolbarHelper::save('careerlevel.savejobcareerlevel');
            if ($isNew)
                ToolbarHelper::cancel('careerlevel.cancel');
            else
                ToolbarHelper::cancel('careerlevel.cancel', 'Close');
            if(isset($application)) $this->application = $application;
        }elseif ($layoutName == 'careerlevels') {        //job types
            ToolbarHelper::title(Text::_('Career Levels'));
            ToolbarHelper::addNew('careerlevel.editcareerlevels');
            ToolbarHelper::editList('careerlevel.editcareerlevels');
            ToolbarHelper::deleteList(Text::_("Are You Sure?"), 'careerlevel.remove');
