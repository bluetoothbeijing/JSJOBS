...
//        layout start
        if ($layoutName == 'formshift') {          // shifts
            ...
            ToolbarHelper::title(Text::_('Shift') . ': <small><small>[ ' . $text . ' ]</small></small>');
            ToolbarHelper::apply('shift.savejobshiftsave', Text::_('Save'));
            ToolbarHelper::save2new('shift.savejobshiftandnew');
            ToolbarHelper::save('shift.savejobshift');
            if ($isNew)
                ToolbarHelper::cancel('shift.cancel');
            else
                ToolbarHelper::cancel('shift.cancel', 'Close');
                if(isset($application)) $this->application = $application;
        }elseif ($layoutName == 'shifts') {        //shifts
            ToolbarHelper::title(Text::_('Shifts'));
            ToolbarHelper::addNew('shift.edijobshift');
            ToolbarHelper::editList('shift.edijobshift');
            ToolbarHelper::deleteList(Text::_('Are You Sure?'), 'shift.remove');
