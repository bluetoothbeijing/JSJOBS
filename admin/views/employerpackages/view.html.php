...
//        layout start
        if ($layoutName == 'formemployerpackage') {          // employer packages
            ...
            ToolbarHelper::title(Text::_('Employer Package') . ': <small><small>[ ' . $text . ' ]</small></small>');
            $this->package = $result[0];
            $this->lists = $result[1];
            ToolbarHelper::save('employerpackages.saveemployerpackage');
            if ($isNew)
                ToolbarHelper::cancel('employerpackages.cancel');
            else
                ToolbarHelper::cancel('employerpackages.cancel', 'Close');
        }elseif ($layoutName == 'employerpackages') {        //employer packages
            ToolbarHelper::title(Text::_('Employer Packages'));
            ToolbarHelper::addNew('employerpackages.add');
            ToolbarHelper::editList('employerpackages.add');
            ToolbarHelper::deleteList(Text::_('Are You Sure?'), 'employerpackages.remove');
