...
<div id="js_menu_wrapper">
    ...
    if (is_array($this->jobseekerlinks) && sizeof($this->jobseekerlinks) != 0) {
        foreach ($this->jobseekerlinks as $lnk) {
            ?>                     
            <a class="js_menu_link <?php if ($lnk[2] == 'login' || $lnk[2] == 'logout') echo 'js_menu_right_link';  if ($lnk[2] == 'login' || $lnk[2] == 'logout') echo 'js_menu_right_link';  if ($lnk[2] == 'coverletter') echo 'selected'; ?>" href="<?php echo $lnk[0]; ?>"><?php echo $lnk[1]; ?></a>
            <?php
        }
    }
...
if ($this->config['offline'] == '1') {
    ...
    <script language="javascript">
        function myValidate(f) {
            if (document.formvalidator.isValid(f)) {
                ...
                return confirm('<?php echo Text::_("Do you sure to submit the erase data request?"); ?>');
            } else {
                alert("<?php echo Text::_("Some values are not acceptable").'. '.Text::_("Please retry"); ?>");
                return false;
            }
    ...
    <?php
        if (isset($this->erasedaatrequest->id)) {
           $heading = "Edit Erase Data Request";
        }else{
           $heading = "Erase Data Request";
        }    
    ?>  
        <div id="jsjobs-main-wrapper">
            ...
            <?php } ?>
    <?php
        if ($this->erase_data_allowed == VALIDATE) { // employer
        ?>
        <!-- Your Area starts here --> 
            <div class="js-jobs-top-search-wrp">
                <div class="js-jobs-search-heading-wrp">
                    <div class="js-jobs-heading-left">
                        <?php echo Text::_("Export your data"); ?>
                    </div>
                    <div class="js-jobs-heading-right">
                        <a class="js-jobs-add-download-btn" href="index.php?option=com_jsjobs&task=gdpr.exportusereraserequest&<?php echo Factory::getSession()->getFormToken(); ?>=1"><span class="js-jobs-add-img-wrp"></span><?php echo Text::_("Export"); ?></a>
                    </div>
                </div>
            </div>
            <?php if(isset($this->erasedaatrequest) && is_numeric($this->erasedaatrequest->id)){ ?>
                <div class="js-jobs-top-search-wrp second-style">
                    <div class="js-jobs-search-heading-wrp second-style">
                        <div class="js-jobs-heading-left">
                            <?php echo Text::_('You have filed a request to remove your data.') ?>
                        </div>
                        <div class="js-jobs-heading-right">
                            <a class="js-jobs-add-download-btn" href="index.php?option=com_jsjobs&task=gdpr.removeusereraserequest&id=<?php echo $this->erasedaatrequest->id; ?>&<?php echo Factory::getSession()->getFormToken(); ?>=1"><span class="js-jobs-add-img-wrp"></span><?php echo Text::_('To withdraw erases data request') ?></a>
                        </div>
                    </div>
                </div>
            <?php }else{ ?>
                ...
                <form action="index.php" method="post" name="adminForm" id="adminForm" class="jsautoz_form" onSubmit="return myValidate(this);">
                    <div class="fieldwrapper">
                        <div class="fieldtitle">
                            <label id="subject" for="name"><?php echo Text::_("Subject"); ?>&nbsp;<font color="red">*</font></label>
                        </div>
                        <div class="fieldvalue">
                            <input class="inputbox-required" type="text" name="subject" id="subject"  value="<?php echo isset($this->erasedaatrequest) ? $this->erasedaatrequest->subject : ''; ?>" required/>
                        </div>
                    </div>                      
                    <div class="fieldwrapper">
                        <div class="fieldtitle">
                            <label id="descriptionmsg" for="message"><?php echo Text::_("Message"); ?>&nbsp;<font color="red">*</font></label>
                        </div>
                        <div class="fieldvalue">
                            <?php
                    ...
                    <div class="fieldwrapper-btn">
                        ...
                        <input type="submit" id="button" class="button jsjobs_button" value="<?php echo Text::_('Submit Request'); ?>"/>
                        </span>
                        </div>
                    </div>
                    ...
                    <input type="hidden" name="layout" value="adderasedatarequestemp" />
...
<!-- End your area -->
        ...
        switch ($this->erase_data_allowed) {
